...
async function boot() {
  ...
  try {
    ...
    if (!res.ok || !data.ok) throw new Error(data.error || `Could not load the project (${res.status}).`);
...
function summaryStep() {
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="chips">
        ${['Safe', 'SWARMP', 'Unsafe']
          .map((s) => `<button type="button" class="chip" data-set="score" data-val="${s}" aria-pressed="${state.score === s}">${esc(s)}</button>`)
          .join('')}
      </div>
    </div>
